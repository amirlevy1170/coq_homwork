...
(** **** Exercise: 3 stars, standard, optional (list_exercises)
...
(** **** Exercise: 5 stars, advanced (all_but_last)

    Write a function [all_but_last] that takes a natlist, and returns 
   that list without the last element. When given the empty list, it 
   should return the empty list.

   Write two versions of that function: [all_but_last1] using recursion
   (in [Fixpoint] statement), and [all_but_last2] without recursion (in
   [Definition] statement) by using functions we saw before on
   natlists.

   Prove a few unit tests to both functions, and then formally prove them
   equivalent. The latter proof can be challenging! You may need to come
   up with helper lemmas, prove them, and use them in your proof. You
   may also get stuck when Coq refuses to open [all_but_last2]. To force
   it to open you can use the tactic [unfold all_but_last2], which we
   will learn later on.

*)

Fixpoint all_but_last1 (l:natlist) : natlist :=
  match l with
  | [] => []
  | h::[] => []
  | h::t => [h]++all_but_last1 t
end.

Definition all_but_last2 (l:natlist) : natlist :=
  match rev(l) with
  | [] => []
  | h :: t => rev t
end.

Example test_all_but_last1_1:  all_but_last1 [12; 12; 14] = [12 ; 12].
Proof. simpl. reflexivity. Qed.

Example test_all_but_last1_2:  all_but_last1 [1] = [].
Proof. simpl. reflexivity. Qed.

Example test_all_but_last1_3:  all_but_last1 [] = [].
Proof. simpl. reflexivity. Qed.

Example test_all_but_last2_1:  all_but_last2 [12; 12; 14] = [12 ; 12].
Proof. simpl. reflexivity. Qed.

Example test_all_but_last2_2:  all_but_last2 [1] = [].
Proof. simpl. reflexivity. Qed.

Example test_all_but_last2_3:  all_but_last2 [] = [].
Proof. simpl. reflexivity. Qed.

(* FILL IN HERE *)

Lemma helper_1 : forall (h1 h2 : nat) (l : natlist),
  all_but_last1 (h1 :: h2 :: l) = h1::all_but_last1(h2::l).
Proof.
  intros. simpl. reflexivity. Qed.

Lemma helper_2 : forall (h1 h2 : nat) (l : natlist),
  h1::all_but_last2 (h2 :: l) = all_but_last2(h1::h2::l).
Proof. 
Admitted.



Lemma all_but_last_equiv : forall (l : natlist), 
  all_but_last1 l = all_but_last2 l.
Proof.
  intros. induction l as [| l' l''].
  - simpl. reflexivity.
  - induction l'' as [| li]. + simpl. reflexivity.
          + rewrite helper_1. rewrite IHl''. rewrite helper_2. reflexivity.
Qed.
(** [] *)

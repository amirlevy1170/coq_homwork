...
- exfalso. apply (while_true_nonterm b c st st' H);auto.
 - inversion H0; subst.
    + inversion H5.
    + exfalso.
      apply (while_true_nonterm BTrue CSkip st st'); try unfold bequiv; auto.
Qed.
...
Theorem assign_aequiv : forall (X : string) (a : aexp),
  ...
  cequiv <{ skip }> <{ X := a }>.
Proof.
unfold aequiv; intros; split; intros; inversion H0; subst.
- assert (st' = t_update st' X (st' X)); try rewrite t_update_same; auto.
rewrite H1 at 2. apply E_Asgn. symmetry. apply H.
- assert (st = t_update st X (aeval st a)); try rewrite t_update_same; auto.
 +  apply functional_extensionality. intros. rewrite <- H. simpl. rewrite t_update_same; auto.
 +  rewrite <-  H1. apply E_Skip; auto.
Qed. 
(** [] *)
...
Proof.
  ...
  split; intros.
  - remember <{ c1;c2 }> as H2.
destruct H1; inversion HeqH2; subst; try apply E_Seq with (st' := st').
...
(** **** Exercise: 5 stars, advanced (com_congruence)

    In this exercise we will formalize the concept of a program context
    for an arithmetic expression and use it to prove an aspect of the fact
    that the command equivalence relation is a congruence relation. *)

Module BoolCtx.
(** To guide you, we tackle boolean contexts first, by
    replicating the definition of [bexp], but with a new "Hole"
    constructor. The idea is to later plug a [bexp] into this
    hole to get a [bexp] (without a hole). *)
...
(** Remark: Nothing forces [bctx]s to have exactly one hole.
    A more precise definition could enforce this, but it would be
    more complicated. Our theorem will be slightly more general
    than what we set out to prove. *)

Declare Custom Entry cctx.
Declare Scope cctx_scope.

Open Scope cctx_scope.

Notation "<[ e ]>" := e (at level 0, e custom cctx at level 99) : cctx_scope.
Notation "( x )" := x (in custom cctx, x at level 99) : cctx_scope.
Notation "x" := x (in custom cctx at level 0, x constr at level 0) : cctx_scope.
Notation "f x .. y" := (.. (f x) .. y)
                  (in custom cctx at level 0, only parsing,
                  f constr at level 0, x constr at level 9,
                  y constr at level 9) : cctx_scope.
Notation "x + y"   := (APlus x y) (in custom cctx at level 50, left associativity).
Notation "x - y"   := (AMinus x y) (in custom cctx at level 50, left associativity).
Notation "x * y"   := (AMult x y) (in custom cctx at level 40, left associativity).
Notation "'b?'"  := C_BHole (in custom cctx at level 0).
Notation "'true'"  := true (at level 1).
Notation "'true'"  := C_BTrue (in custom cctx at level 0).
Notation "'false'" := false (at level 1).
Notation "'false'" := C_BFalse (in custom cctx at level 0).
Notation "x <= y"  := (C_BLe x y) (in custom cctx at level 70, no associativity).
Notation "x > y"   := (C_BGt x y) (in custom cctx at level 70, no associativity).
Notation "x = y"   := (C_BEq x y) (in custom cctx at level 70, no associativity).
Notation "x <> y"  := (C_BNeq x y) (in custom cctx at level 70, no associativity).
Notation "x && y"  := (C_BAnd x y) (in custom cctx at level 80, left associativity).
Notation "'~' b"   := (C_BNot b) (in custom cctx at level 75, right associativity).

Definition example_bexp : bctx := <[ b? && ~(X <= 4) ]>.

(** We need to allow the hole to appear anywhere within a command.
    We can use [aexp] as-is because booleans never appear there,
    but we have to expand the definition of [com]. *)

Inductive cctx : Type :=
  | C_CSkip
  | C_CAsgn (x : string) (a : aexp)
  | C_CSeq (cc1 cc2 : cctx)
  | C_CIf (cb : bctx) (cc1 cc2 : cctx)
  | C_CWhile (cb : bctx) (cc : cctx)
.

Notation "'skip'"  :=
         C_CSkip (in custom cctx at level 0) : cctx_scope.
Notation "x := y"  :=
         (C_CAsgn x y)
            (in custom cctx at level 0, x constr at level 0,
             y at level 85, no associativity) : cctx_scope.
Notation "x ; y" :=
         (C_CSeq x y)
           (in custom cctx at level 90, right associativity) : cctx_scope.
Notation "'if' x 'then' y 'else' z 'end'" :=
         (C_CIf x y z)
           (in custom cctx at level 89, x at level 99,
            y at level 99, z at level 99) : cctx_scope.
Notation "'while' x 'do' y 'end'" :=
         (C_CWhile x y)
            (in custom cctx at level 89, x at level 99, y at level 99) : cctx_scope.

(** We define plugging boolean holes with booleans and show congruence. *)

Fixpoint bctx_plug (bc : bctx) (b : bexp) : bexp :=
  match bc with
  | <[b?]> => b
  | <[true]> => <{true}>
  | <[false]> => <{false}>
  ...
  | <[a1 > a2]> => <{a1 > a2}>
  | <[~ cb1]> => <{~ bctx_plug cb1 b}>
  | <[cb1 && cb2]> => <{bctx_plug cb1 b && bctx_plug cb2 b}>
  end.

Theorem bexp_congruence : forall cb b1 b2,
    bequiv b1 b2 ->
    bequiv (bctx_plug cb b1) (bctx_plug cb b2).
Proof.
  intros ca b1 b2 Hb.
  induction ca; try assumption; try apply refl_bequiv;
    intros st; try reflexivity; simpl.
  - rewrite IHca; reflexivity.
  - rewrite IHca1, IHca2; reflexivity.
Qed.

(** We define plugging command holes with booleans and show congruence. *)

Fixpoint cctx_plug (cc : cctx) (b : bexp) : com :=
  match cc with
  | <[ skip ]> => <{ skip }>
  | <[ x := a ]> => <{ x := a }>
  | <[ cc1 ; cc2 ]> => <{ cctx_plug cc1 b ; cctx_plug cc2 b }>
  | <[ if cb then cc1 else cc2 end ]> => <{ if bctx_plug cb b then cctx_plug cc1 b else cctx_plug cc2 b end }>
  | <[ while cb do cc end ]> => <{ while bctx_plug cb b do cctx_plug cc b end }>
  end.

Example cctx_plug_test1 :
  cctx_plug
  <[ while Z <> 0 && ~ b? do Y := Z; Z := Z - 1 end ]> <{X = Y}>
  = <{ while Z <> 0 && ~ (X = Y) do Y := Z; Z := Z - 1 end }>.
Proof. reflexivity. Qed.

(** Prove the boolean aspect of the fact that command equivalence
    is a congruence. *)


Theorem com_congruence : forall cc b1 b2,
    bequiv b1 b2 ->
    cequiv (cctx_plug cc b1) (cctx_plug cc b2).
Proof.
  intros cc b1 b2 Hb.
  induction cc; simpl.
  ...
  - (* C_CIf *)
    apply CIf_congruence.
    + apply bexp_congruence. assumption.
    + assumption.
    + assumption.
  - (* C_CWhile *)
    apply CWhile_congruence.
    + eapply bexp_congruence. auto.
    + assumption.
Qed.

(** Here is an example of how we can use the theorem: *)

Example btcx_cequiv1 :
    cequiv
    <{ while X <= 0 do X := X + 1 end }>
    <{ while ~ X > 0 do X := X + 1 end }>.
Proof.
  apply (com_congruence <[ while b? do X := X + 1 end ]>).
  intros st.
  simpl.
  symmetry.
...
(** Define commands with holes for an arithmetic expression.
    Holes must be allowed to appear anywhere an arithmetic
    expression may appear. You may need additional
    prior definitions. *)
...
(** You can try to recreate the notation, but don't waste too
    much time on this. We do not require understanding Coq's
    notational devices and it is not part of the exercise. *)

(* FILL IN HERE *)

(** Define a function that plugs a given command, boolean expression,
    and arithmetic expression into a given program context. *)

Fixpoint cctx_plug (cc : cctx) (a : aexp) : com
  (* REPLACE THIS LINE WITH ":= _your_definition_ ." *). Admitted.

(** The following test should work if you managed to replicate
    the notation. If not, you can rewrite it without the fancy
    notation to test your definition. *)

(* 
Example cctx_plug_test1 :
  cctx_plug
  <[ while Z <> 0 do Y := a? * Z; Z := Z - 1 end ]> <{1}>
  = <{ while Z <> 0 do Y := 1 * Z; Z := Z - 1 end }>.
Proof. (* FILL IN HERE *) Admitted.

Example cctx_plug_test2 :
  cctx_plug
  <[ while Z <> a? do Y := 1 * Z; Z := Z - 1 end ]> <{0}>
  = <{ while Z <> 0 do Y := 1 * Z; Z := Z - 1 end }>.
Proof. (* FILL IN HERE *) Admitted.
*)

(** Prove the arithmetic aspect of the fact that command equivalence
    is a congruence. *)

Theorem com_congruence : forall cc a1 a2,
    aequiv a1 a2 ->
    cequiv (cctx_plug cc a1) (cctx_plug cc a2).
Proof.
  (* FILL IN HERE *) Admitted.

(** You are welcome to try to use the theorem in the following
    example: *)

(* 
Example btcx_cequiv1 :
    cequiv
    <{ while X <= 1 - 1 do X := X + 1 end }>
    <{ while X <= 0 do X := X + 1 end }>.
Proof.
  apply (com_congruence <[ while X <= a? do X := X + 1 end ]>).
  intros st.
  simpl.
  reflexivity.
Qed.
...
(** **** Exercise: 4 stars, standard (better_subst_equiv)
...
Proof.
  intros.
  induction a; simpl; inversion H;
    try reflexivity.
 - apply (t_update_neq _  st  x x0 ni H1).
 - subst. apply IHa1 in H2; apply IHa2 in H3.
...
Definition var_not_used_in_aexp_subst_equiv_property : Prop := forall x1 x2 a1 a2,
  var_not_used_in_aexp x1 a1 ->
  cequiv <{ x1 := a1; x2 := a2 }>
         <{ x1 := a1; x2 := subst_aexp x1 a1 a2 }>.


(** i tried to solve this one as a helper function but couldnt make it unfortunately 
i know i need to use aeval_weakening where i admitted.... apply (aeval_weakening x st a1 0) in H.
 *)
Check String.eqb_neq.

Lemma neq_string_eqb_false : forall (i x : string),
  i <> x ->
  (i =? x)%string = false.
Proof.
  intros i x Hneq.
  destruct (string_dec i x) as [Heq | Hneq'].
  - contradiction.
  - apply String.eqb_neq. auto.
Qed.

Lemma eq_string_true : forall i x : string, 
  (i =? x)%string = true -> x = i.
Proof.
  intros i x Heq.
...
Lemma helper : forall st i a1 a2,
  var_not_used_in_aexp i a1 ->
  aeval (t_update st i (aeval st a1)) (subst_aexp i a1 a2) =
  aeval (t_update st i (aeval st a1)) a2.
Proof.
  intros st i a1 a2 H.
  induction a2; simpl; try reflexivity.
  - destruct (i =? x)%string eqn:e1.
  + apply eq_string_true in e1. subst. 
   apply (aeval_weakening i st a1 (aeval st a1)) in H.
    rewrite H. 
    rewrite t_update_eq.
    auto.
  + simpl. auto.

-rewrite IHa2_1, IHa2_2; reflexivity.
-rewrite IHa2_1, IHa2_2; reflexivity.
-rewrite IHa2_1, IHa2_2; reflexivity.
Qed.

Theorem var_not_used_in_aexp_subst_equiv_property_proof : 
var_not_used_in_aexp_subst_equiv_property.
Proof.
  split; intros;
  
    inversion H0; subst;
    apply E_Seq with st'0; try assumption;
      inversion H3; inversion H6; subst; apply E_Asgn.
      apply helper; assumption.
      symmetry; apply helper; assumption.
Qed.

(* Do not modify the following line: *)
Definition manual_grade_for_better_subst_equiv : option (nat*string) := None.
(** [] *)
...
Theorem swap_noninterfering_assignments: forall l1 l2 a1 a2,
  ...
  cequiv
    ...
    <{ l2 := a2; l1 := a1 }>.
Proof.
  intros l1 l2 a1 a2 Hneq Hnu1 Hnu2.

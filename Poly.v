...
Lemma helper : forall (X Y : Type) (f : X -> Y) (l k : list X),
...
intros. induction l.
  - reflexivity.
  - simpl. rewrite -> helper. simpl. rewrite IHl. reflexivity.
Qed.
(** [] *)
...
(** **** Exercise: 2 stars, standard (trees) *)

(** Recall the binary trees we saw in Rec02.v.
...
(** Define a function [foldtree] that is similar to [fold],
    such that [listify] below flattens the tree into a list.*)
...
Fixpoint foldtree {X A : Type} (f : X -> A -> A) (a : A) (t : tree X) : A :=
  match t with
    | Tree_None k => f k a
    | Tree_Both k l r => f k (foldtree f (foldtree f a r) l)
    | Tree_Left k l => f k (foldtree f a l)
    | Tree_Right k r => f k (foldtree f a r)
  end.

Notation "l // n \\ r" := (Tree_Both n l r)
                       (at level 50)
                       : nat_scope.

Notation "n -\\ r" := (Tree_Right n r)
                       (at level 60)
                       : nat_scope.

Notation "l //- n " := (Tree_Left n l)
                       (at level 55)
                       : nat_scope.

Notation "# n" := (Tree_None n)
                       (at level 35)
                       : nat_scope.

Definition listify {X : Type} : tree X -> list X := @foldtree X (list X) cons nil.

Example test_foldtree1 : listify (#4) = [4].
Proof. simpl. reflexivity. Qed.

Example test_foldtree2 : listify ((2-\\#5) // 7 \\ #4) = [2; 5; 7; 4].
Proof. simpl. Admitted.

Example test_foldtree3 : listify ((2-\\#5) // 7 \\ (#1 // 9 \\ #8)) = [2; 5; 7; 1; 9; 8].
Proof. simpl. Admitted.

Example test_foldtree4 : listify ((2-\\(#0 // 5 \\ #4)) // 7 \\ (#1 // 9 \\ #8)) = [2; 0; 5; 4; 7; 1; 9; 8].
Proof. simpl. Admitted.
...
(** **** Exercise: 5 stars, advanced (last_error) *)

(* 1) Write a similar recursive function that gets a polymorphic list, and
   returns the last element of that list (where None signifies an empty list).
   2) Add some manual tests to test your function.
   3) Write another version of the same function using fold
      (in a Definition statement).
   4) Prove the equivalence of the two versions.
      In you proof, it may be useful to note that the simplification
      done by "reflexivity" is stronger than the one in "simpl"
      (reflexivity will also unfold definitions for you).
*)

Fixpoint last_elem {X : Type} (l : list X) : option X :=
  match l with
  | [] => None
  | [x] => Some x
  | _ :: t => last_elem t
...
Example firstTry: last_elem [2;3;4;5] = Some(5).
Proof. reflexivity. Qed.

Example secondTry : last_elem ([] : list nat) = None.
Proof. reflexivity. Qed.



Definition last_elem_reg {X : Type} (l : list X) : option X :=
  fold (fun x acc => Some x) (rev l) None.


Compute last_elem_reg [2;3;4;5].

Example firstTry2: last_elem_reg [2;3;4;5] = Some(5).
Proof. reflexivity. Qed.

Example secondTry2 : last_elem_reg ([] : list nat) = None.
Proof. reflexivity. Qed.



Theorem last_elem_eq: forall (X : Type) (l : list X),
last_elem l = last_elem_reg l.
Proof.
  intros X l.
...
(* Do not modify the following line: *)
Definition manual_grade_for_last_error : option (nat*string) := None.
(** [] *)

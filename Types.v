...
(** **** Exercise: 3 stars, standard, optional (finish_progress) *)
...
(** **** Exercise: 2 stars, standard, optional (finish_preservation) *)
...
(** **** Exercise: 3 stars, standard, optional (preservation_alternate_proof)
...
(** **** Exercise: 3 stars, standard, optional (subject_expansion)
...
(** **** Exercise: 2 stars, standard, optional (variation2)
...
(** **** Exercise: 1 star, standard, optional (remove_pred0)
...
(** **** Exercise: 5 stars, standard (coerce_condition) *)

(** In this exercise we define a language in which numbers can
    also be used within conditionals, with [0] understood as [false]. *)
...
(** We keep the same syntax except for removing [iszero]: *)
...
Declare Custom Entry ctm.
Declare Scope ctm_scope.
Notation "'true'"  := true (at level 1): ctm_scope.
Notation "'true'" := (tru) (in custom ctm at level 0): ctm_scope.
Notation "'false'"  := false (at level 1): ctm_scope.
Notation "'false'" := (fls) (in custom ctm at level 0): ctm_scope.
Notation "<[ e ]>" := e (e custom ctm at level 99): ctm_scope.
Notation "( x )" := x (in custom ctm, x at level 99): ctm_scope.
Notation "x" := x (in custom ctm at level 0, x constr at level 0): ctm_scope.
Notation "'0'" := (zro) (in custom ctm at level 0): ctm_scope.
Notation "'0'"  := 0 (at level 1): ctm_scope.
Notation "'succ' x" := (scc x) (in custom ctm at level 90, x custom ctm at level 80): ctm_scope.
Notation "'pred' x" := (prd x) (in custom ctm at level 90, x custom ctm at level 80): ctm_scope.
Notation "'if' c 'then' t 'else' e" := (ite c t e)
                 (in custom ctm at level 90, c custom ctm at level 80,
                  t custom ctm at level 80, e custom ctm at level 80): ctm_scope.
Local Open Scope ctm_scope.

Inductive bvalue : ctm -> Prop :=
  | bv_true : bvalue <[ true ]>
  | bv_false : bvalue <[ false ]>.

Inductive nvalue : ctm -> Prop :=
  | nv_0 : nvalue <[ 0 ]>
  | nv_succ : forall t, nvalue t -> nvalue <[ succ t ]>.

Definition value (t : ctm) := bvalue t \/ nvalue t.
...
(** We remove the rules concerning [iszero] (that start with [ST_Iszero])
    from the step relation and add the following rules:

                           numeric value v
                   --------------------------------                    (ST_If>0)
                   if succ v then t1 else t2 --> t1

                   ---------------------------                         (ST_If=0)
                   if   0    then t1 else t2 --> t2
*)

Reserved Notation "t '-->' t'" (at level 40).

Inductive step : ctm -> ctm -> Prop :=
  | ST_IfZro : forall t1 t2,
      <[ if 0 then t1 else t2 ]> --> t1
  | ST_IfPos : forall v t1 t2,
      nvalue v ->
      <[ if succ v then t1 else t2 ]> --> t2
  | ST_IfTrue : forall t1 t2,
      <[ if true then t1 else t2 ]> --> t1
  | ST_IfFalse : forall t1 t2,
      <[ if false then t1 else t2 ]> --> t2
...
Definition stuck (t : ctm) : Prop :=
  normal_form step t /\ ~ value t.

Hint Unfold stuck : core.

(** Define the [translation] function such that it preserves the values
    that terms evaluate to. *)

Fixpoint translation (t : tm) : ctm
  (* REPLACE THIS LINE WITH ":= _your_definition_ ." *). Admitted.

Example translation_test1 : translation <{ iszero (pred (succ 0)) }> -->* <[ true ]>.
Proof.
  (* FILL IN HERE *) Admitted.

Example translation_test2 : translation <{ iszero (succ (pred 0)) }> -->* <[ false ]>.
Proof.
  (* FILL IN HERE *) Admitted.

Example translation_test3 : translation <{ if succ 0 then iszero 0 else iszero (succ 0) }> -->* <[ false ]>.
Proof.
  (* FILL IN HERE *) Admitted.
...
(** Define a typing relation for this new language. As in the original
    language, a term's type should be unique if it exists. Additionally,
    progress and type preservation should hold.
    Do so first informally, describing which rules should be removed and
    which should be added to the system of the original language,
    and then give the formal definition as well. *)

(** We remove the rule concerning [iszero] (i.e. [T_Iszero])
    from the typing relation and add the following rule:

           |-- t1 \in Nat     |-- t2 \in T    |-- t3 \in T
           -----------------------------------------------     (T_IfNat)
                   |-- if t1 then t2 else t3 \in T
*)
(** For good measure, we also rename [T_If] to [T_IfBool]. *)

Reserved Notation "'|--' t '\in' T" (at level 40).

Inductive has_type : ctm -> ty -> Prop :=
  (* FILL IN HERE *)
...
Example has_type_1 :
  |-- <[ if 0 then false else true ]> \in Bool.
Proof.
  (* FILL IN HERE *) Admitted.
...
(** Prove uniqueness of typing: *)

Proposition unique_typing : forall t T1 T2,
    |-- <[ t ]> \in T1 ->
    |-- <[ t ]> \in T2 ->
                    T1 = T2.
Proof.
...
(** Prove progress and preservation below. We recommend copying the
    proofs from the original language and modifying them rather than
    starting from scratch, as they should be mostly the same. *)

...
(** **** Exercise: 3 stars, standard (nandb_complete) *)

(** Recall the [nandb] function that you defined above.
    This logical connective is "complete", in the sense that it allows
    one to express all standard logical connectives.
    
    Find a way to express [andb] using only [nand], that is 
    complete the following definitions using only the following symbols:
    [nandb], [b1], [b2], [(], and [)].
    Then, prove correctness.

    If needed, we suggest to take a piece of paper to think about it.
**)

Definition andb_using_nandb (b1:bool) (b2:bool) : bool 
  (* REPLACE THIS LINE WITH ":= _your_definition_ ." *). Admitted.

Theorem andb_using_nandb_correct : forall b c : bool,
  andb_using_nandb b c = andb b c.
Proof.
  (* FILL IN HERE *) Admitted.
(** [] *)

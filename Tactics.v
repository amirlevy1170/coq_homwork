...
injection H1 as H1a H1b. rewrite H2 in H1b. injection H1b as H1ba.
rewrite H1a. symmetry. apply H1ba.
Qed.
...
(** Another handy tactic for fiddling with hypotheses is [specialize].
    ...
    change [H] so that it becomes [P] with [x] replaced by [e]. *)
...
Proof.
  intros n X l.
  generalize dependent n.
  induction l as [| x xs IHxs]; intros n Hlen.
  - simpl in Hlen. rewrite <- Hlen. reflexivity.
  - destruct n.
    + simpl in Hlen. discriminate Hlen.
    + simpl in Hlen. apply IHxs. injection Hlen as Hlen'. apply Hlen'.
Qed.
(** [] *)
...
- intros l1 l2 H1. simpl in H1. injection H1 as H1a H1b. subst. reflexivity.
- intros l1 l2 H1. simpl in H1. destruct (split l') as [l1' l2'] eqn:Heq.
injection H1 as H1a H1b. subst. simpl. rewrite IHl. 
+ reflexivity.
+ reflexivity.
Qed.
(** [] *)
...
(** **** Exercise: 3 stars, advanced (injection_without_injection)

    The following injectivity claims should be very easy using
    the [injection] tactic. Your goal is to prove them without
    using [injection].
    Hint: try to mimic what we did in S_injective.
          You also may want to use helper definitions. *)
Theorem S_injectiv : forall (n m : nat),
  S n = S m ->
  n = m.
Proof.
  intros n m H1.
...
Theorem Some_injective : forall (X : Type)
  (x1 x2 : X), Some x1 = Some x2 -> x1 = x2.
Proof.
  intros. inversion H. reflexivity. 
Qed.

Theorem cons_injective_head : forall (X : Type)
  (h1 h2 : X) (t1 t2 : list X),
  cons h1 t1 = cons h2 t2 -> h1 = h2.
Proof.
  intros. inversion H. reflexivity.
Qed.

Theorem cons_injective_tail : forall (X : Type)
  (h1 h2 : X) (t1 t2 : list X),
  cons h1 t1 = cons h2 t2 -> t1 = t2.
Proof.
  intros. inversion H. reflexivity.
Qed.
...
(** **** Exercise: 4 stars, advanced, optional (sum_tail)

    Recall the function [rev_tail] we have defined in [Rec02.v].
    Define a tail-recursive version of [sum]. Recall that a function is
    tail-recursive if the recursive call(s) complete the computation.
    Do not change the headers (i.e. the type declarations). *)
...
Fixpoint sum_tail_add (acc : nat) (l : list nat) : nat
  (* REPLACE THIS LINE WITH ":= _your_definition_ ." *). Admitted.

Definition sum_tail : list nat -> nat
  (* REPLACE THIS LINE WITH ":= _your_definition_ ." *). Admitted.

Example test_sum_tail1 : sum_tail_add 1 [2;3] = 6.
Proof. (* FILL IN HERE *) Admitted.

Example test_sum_tail2 : sum_tail_add 0 [2;3] = 5.
Proof. (* FILL IN HERE *) Admitted.

Example test_sum_tail3 : sum_tail [2;3] = 5.
Proof. (* FILL IN HERE *) Admitted.

Example test_sum_tail4 : sum_tail [] = 0.
Proof. (* FILL IN HERE *) Admitted.

(** Prove the theorem below using the lemma, thus showing that
    [sum_tail] is equivalent to [sum]. You may find theorems
    about [+] from Induction.v helpful, such as [add_shuffle3]. *)

Lemma sum_tail_equiv_helper : forall (acc : nat) (l : list nat),
     sum_tail_add acc l = sum l + acc.
Proof.
  (* FILL IN HERE *) Admitted.

Theorem sum_tail_equiv : forall (l : list nat),
    sum_tail l = sum l.
Proof.
  (* FILL IN HERE *) Admitted.
...
(** **** Exercise: 4 stars, advanced, optional (map_eq_app) *)

(** Prove the following properties of append and map.
   In the first two, you don't need induction at all. *)

Lemma app_eq_nil_r : forall (A : Type) (l1 l2 : list A),  l1 ++ l2 = [] -> l2 = [].
Proof. (* FILL IN HERE *) Admitted.

Lemma map_eq_app : forall (A B : Type) (f : A -> B) (l1 l2 : list A) (l1' l2' : list B) (l : list A),
l = l1 ++ l2 -> map f l1 = l1' -> map f l = l1' ++ l2' -> map f l2 = l2'.
Proof. (* FILL IN HERE *) Admitted.

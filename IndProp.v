...
| allOptionsGood : forall n m, total_relation n m.

Theorem total_relation_is_total : forall n m, total_relation n m.
  Proof.
intros. apply allOptionsGood. Qed.
...
|subseq_add : forall x l1 l2, subseq l1 l2 -> subseq l1 (x::l2)
|subseq_good : forall x l1 l2, subseq l1 l2 -> subseq (x::l1) (x::l2).
...
- simpl. apply subseq_good. apply IHsubseq.
Qed.

Lemma bigger_subseq : forall (x x0:nat) (l1 l2 : list nat),
subseq (x::l1) l2 -> subseq (x::l1)(x0::l2).
Proof. intros. induction H. 
- apply subseq_Empty.
- apply subseq_add. apply subseq_add. apply H.
- apply subseq_add. apply subseq_good. apply H.
Qed.

Lemma smaller_subseq : forall (x:nat) (l1 l2 : list nat),
...
- intros. apply subseq_add. apply IHsubseq. apply H.
- intros. inversion H.
  + apply subseq_add, subseq_Empty.
  + apply subseq_add, IHsubseq, H3.
  + apply subseq_good, IHsubseq, H3.
Qed.
...
| nilSut: nostutter []
| oneSut (x:X) : nostutter (x :: [])
| consSut (x y: X) (l: list X) : 
    x <> y -> nostutter (x :: l) -> nostutter (y :: x :: l).
...
Lemma in_app_or : forall (A : Type) (l1 l2 : list A) (a : A),
...
Theorem disjoint_NoDup_append : forall (X : Type) (l1 l2 : list X),
  disjoint l1 l2 -> NoDup l1 -> NoDup l2 -> NoDup (l1 ++ l2).
Proof.
intros X l1 l2 Hdisjoint HNoDup1 HNoDup2.
induction HNoDup1. - simpl. assumption. - simpl. constructor.
 + intro H1. apply in_app_or in H1. destruct H1 as [H2 | H3].
    * contradiction.
    * apply Hdisjoint in H3. contradiction. simpl. left. reflexivity.
 + apply IHHNoDup1. intros x1 H1. apply Hdisjoint. right. apply H1.
Qed.
...
(** **** Exercise: 7 stars, standard (perfect_tree_size)

    Recall the polymorphic binary trees from the Poly
    chapter. *)

Inductive tree (X : Type) : Type :=
| Tree_None  (x : X) : tree X
| Tree_Both  (x : X) : tree X -> tree X -> tree X
| Tree_Left  (x : X) : tree X -> tree X
| Tree_Right (x : X) : tree X -> tree X.

Arguments Tree_None  {X}.
Arguments Tree_Both  {X}.
Arguments Tree_Left  {X}.
Arguments Tree_Right {X}.

Notation "l // x \\ r" := (Tree_Both x l r)
                       (at level 50)
                       : nat_scope.

Notation "x -\\ r" := (Tree_Right x r)
                       (at level 60)
                       : nat_scope.

Notation "l //- x " := (Tree_Left x l)
                       (at level 55)
                       : nat_scope.

Notation "# x" := (Tree_None x)
                       (at level 35)
                       : nat_scope.

Definition example := (2-\\ # 5) // 7 \\ (# 4).

(** We also defined a height function: *)

Fixpoint height {X : Type} (bt : tree X) : nat :=
  match bt with
    | Tree_None _ => 0
    | Tree_Both _ l r => S (max (height l) (height r))
    | Tree_Left _ l => S (height l)
    | Tree_Right _ r => S (height r)
  end.

(** Your goal is to write an inductive predicate that holds
    iff the tree is perfect. A perfect binary tree is a binary
    tree in which every internal node has exactly two child
    nodes and all of the leaf nodes are at the same level. *)

Inductive perfect {X : Type} : tree X -> Prop :=
  | perfect_none : forall x, perfect (Tree_None x)
  | perfect_both : forall x l r, perfect l -> perfect r -> height l = height r -> perfect (Tree_Both x l r).


Example perfect_test1 : perfect ((# 5) // 7 \\ (# 4)).
Proof. apply perfect_both.
- apply perfect_none.
- apply perfect_none.
- simpl. reflexivity.
Qed. 

Example perfect_test2 : ~ perfect (((# 5) // 7 \\ (# 4)) // 7 \\ (# 4)).
Proof.
  intros C. remember (((# 5) // 7 \\ (# 4)) // 7 \\ (# 4)) as bt.
inversion C.
- symmetry in H. rewrite H in Heqbt. inversion Heqbt.
-  symmetry in H2. rewrite H2 in Heqbt. inversion Heqbt.
rewrite H5 in H1. rewrite H6 in H1. simpl in H1. inversion H1.
Qed. 

(** Next, define a size function for trees that recursively
    counts the number of nodes in a tree, including the leaves. *)

Fixpoint size {X : Type} (bt : tree X) : nat :=
  match bt with
    | Tree_None _ => 1
    | Tree_Both _ l r => 1 + size l + size r 
    | Tree_Left _ l => 1 + size l
    | Tree_Right _ r => 1 + size r
  end.

Example size_test : size ((2-\\ # 5) // 7 \\ (# 4)) = 4.
Proof. simpl. reflexivity. Qed.

(** If the tree is perfect, can the size be calculated
    from the height? Fill here:
*)
Check exp. 
Compute exp 2 6.

Definition size' {X : Type} (bt : tree X) : nat:=
pred(exp 2 (S(height bt))) .

Example size_test' : size ((# 5) // 7 \\ (# 4)) = 3.
Proof. simpl. reflexivity. Qed.

(** You may use the function "exp" from the standard library
    for exponentiation. Try some simple unit tests, to see
    that indeed [size bt = size' bt] for every tree [bt]. *)

(* FILL IN HERE *)

(** Finally, prove the following [perfect_size] lemma.
    You may want to break your proof by adding helper lemmas
    as you need. Also, you may use the following property: *)

Lemma specific_helper : forall k,
  k <> 0 -> pred (k + k + (k + k)) =
  S (pred (k + k) + pred (k + k)).
Proof.
  lia.
...
(** Note that we used the very powerful [lia] tactic,
    that we will learn soon in class.
    You are not allowed to use [lia] yourself in your proof.
    But, if [specific_helper] is not exactly what you need,
    you may change it as you want, as long as [lia] solves
    it. The only use of [lia] should be in the proof of
    [specific_helper]. *)

Lemma mult_plus_distr_l : forall n m p : nat, n * (m + p) = n * m + n * p.
Proof. lia. Qed.

Lemma double_zero : forall n : nat, n + n = 0 -> n = 0.
Proof.
  intros n H.
  destruct n as [| n'].
  - reflexivity.
...
Lemma max_idempotent : forall n : nat, max n n = n.
Proof.
  intros n.
  rewrite max_l.
  - reflexivity.
...
Lemma not0 : forall x : nat , exp 2 x <> 0.
Proof.
  intros x H.
...
-
    simpl in H. apply IHx'. rewrite <- plus_n_O in H.
apply double_zero in H. apply H. 
Qed.

Search (max _ _ = _).

Lemma perfect_size : forall X (bt : tree X),
  perfect bt -> size' bt = size bt.
Proof.
  intros X bt H. induction H as [| x l r Hl IHl Hr IHr Hh].
-    simpl. reflexivity.
-
    simpl. rewrite <- IHl. rewrite <- IHr.
    assert (H : forall n, exp 2 (S n) = 2 * exp 2 n).
    { intros n. simpl. rewrite <- plus_n_O. reflexivity. }
    unfold size'. symmetry in H. rewrite <- H. simpl.  rewrite <- plus_n_O. rewrite <- plus_n_O.
rewrite <- plus_n_O. rewrite <- plus_n_O. rewrite Hh. 
 replace (exp 2 (max (height r) (height r))) with (exp 2 (height r)).
+ apply specific_helper. apply not0.
+ rewrite max_idempotent. reflexivity.

...
(** **** Exercise: 2 stars, standard (typing_example_2_full)
...
(** **** Exercise: 5 stars, advanced (big_step)

    Devise big-step semantics for STLC. The definition should be direct, that is,
    without using the small-step definitions such as [-->], [-->*], and [value].
    Then, prove the lemmas showing it is equivalent to the small-step semantics.
    Hint: you may try to mimic what we did in Smallstep.v for the simple 
          expression language. *)
...
Lemma eval_value : forall t t', t ==> t' -> value t'.
Proof.
  (* FILL IN HERE *) Admitted.

Theorem eval__multistep : forall t t',
  t ==> t' -> t -->* t'.
Proof.
  (* FILL IN HERE *) Admitted.

...
Definition normal_form_of {X : Type}
              (R : relation X) (t t': X) : Prop :=
...
(** We have already seen that, for our language, single-step
    ...
    In other words, we can actually pronounce [normal_form step t t'] as
    "[t'] is _the_ normal form of [t]." *)

(** **** Exercise: 3 stars, standard (normal_forms_unique) *)
Theorem normal_forms_unique : forall (X : Type) (R : relation X),
  deterministic R ->
  deterministic (normal_form_of R).
Proof.
  (* We recommend using this initial setup as-is! *)
  unfold deterministic. unfold normal_form_of.
  intros X R D x y1 y2 P1 P2.
  ...
  - destruct P21.
    + reflexivity.
    + exfalso.
      assert (exists t' : X, R x t') as Hexists.
      { exists y. apply H. }
      contradiction.
...
Proof.
intros. 
induction H0. constructor. econstructor. 
apply ST_Plus2; repeat eauto. auto.
Qed.
(** [] *)
...
- repeat constructor. 
- inversion H; subst. apply IHHs in H2. constructor. auto. auto.
- inversion H0; subst. apply IHHs in H5. constructor. auto. auto.
Qed.
(** [] *)
...
Proof.
unfold normal_form_of. unfold normal_form. intros.
destruct H. induction H.
- apply nf_is_value in H0. inversion H0; subst. exists n. split.
reflexivity. constructor.
- apply IHmulti in H0. destruct H0. destruct H0. exists x0. split. 
  + auto.
  + eapply step__eval. eauto. eauto.
Qed.
(** [] *)
...
(** **** Exercise: 5 stars, standard (right_to_left) *)

(** The small-step semantics defined above is evaluating "left-to-right". 
Here is a similar definition that evaluates "right-to-left": *)

Reserved Notation " t '~~>' t' " (at level 40).

Inductive step_rl : tm -> tm -> Prop :=
  | ST_PlusConstConst_rl : forall n1 n2,
      P (C n1) (C n2) ~~> C (n1 + n2)
  | ST_Plus1_rl : forall t1 t1' v2,
      value v2 ->
      t1 ~~> t1' ->
      P t1 v2 ~~> P t1' v2
  | ST_Plus2_rl : forall t1 t2 t2',
      t2 ~~> t2' ->
      P t1 t2 ~~> P t1 t2'

  where " t '~~>' t' " := (step_rl t t').

Notation " t '~~>*' t' " := (multi step_rl t t') (at level 40).

(** Your goal is to prove that the two semantics coincide, as stated in the next
theorem. The full proof may be quite long, so we allow you to make shortcuts.
Since everything is symmetric, you are welcome to copy from above definitions,
theorems, and lemmas about step and change them to be about step_rl. You can
simply admit these lemmas using [Admitted]. *)
Lemma multistep_congr_1_rl : forall t1 t1' t2,
     t1 --> t1' ->
     P t1 t2 ~~> P t1' t2.
Proof.
Admitted.
...
(** **** Exercise: 2 stars, standard (multistep_congr_2) *)
Lemma multisstep_congr_2_rl : forall v1 t2 t2',
     value v1 ->
     t2 --> t2' ->
     P v1 t2 ~~> P v1 t2'.
Proof.
...
Lemma helper_1 : forall a b , a~~>b -> a-->*b. Proof. Admitted. 
(** Use the following convention when you copy: keep the exact same name,
    except for adding [_rl] to the end of the name (just like [step_rl]). 
*)

Theorem step_lr_rl_equivalent : forall t n,
  t -->* C n <-> t ~~>* C n.
Proof.
  split.
  - intros H. induction H as [h1|h2].
    + constructor.
    + apply multi_trans with (y). 
      * apply multi_R. induction H.
        ** constructor.
        ** apply multistep_congr_1_rl. auto.
        ** apply multisstep_congr_2_rl. auto. auto.
      * auto.
  - intros H. induction H as [h1|h2].
    + constructor.
    + apply multi_trans with (y). 
...
Qed. 
(* Do not modify the following line: *)
Definition manual_grade_for_right_to_left  : option (nat*string) := None.
(** [] *)
...
(** **** Exercise: 2 stars, advanced (multi_end)

    Recall the definition of [multi] from above.
    It is also possible to provide an alternative definition that
    works "from the end". Prove the following simple properties
    of the alternative definition. *)

Inductive multi_end {X : Type} (R : relation X) : relation X :=
  | multi_end_refl : forall (x : X), multi_end R x x
  | multi_end_step : forall (x y z : X),
                    multi_end R x y ->
                    R y z ->
                    multi_end R x z.

Theorem multi_end_R : forall (X : Type) (R : relation X) (x y : X),
    R x y -> multi_end R x y.
Proof.
  intros.  apply (multi_end_step R x x y).
  - apply multi_end_refl.
  - auto.
Qed.

Theorem multi_end_trans :
  forall (X : Type) (R : relation X) (x y z : X),
      multi_end R x y  ->
      multi_end R y z ->
      multi_end R x z.
Proof.
  ...
  induction H.
    - (* multi_refl *) assumption.
    - apply  multi_end_step with y. (* multi_step *)
      + apply IHmulti_end. auto. 
      + auto.
Qed.
 
(** [] *)

(** **** Exercise: 5 stars, advanced (multi_equiv_multi_end)

    A more difficult exercise is to prove the equivalence between
    the two definitions. Feel free to add additional lemmas
    if you need them.
    Hint: define and use something like [clos_trans] from 
    IndProp.v as a mediator. *)

(* FILL IN HERE *)

Lemma multi_equiv_multi_end : forall X (R : relation X) x y, 
  multi R x y <-> multi_end R x y.
Proof.
split;intros.
- induction H.
  + constructor.
  + apply multi_end_R in H. apply multi_end_trans with y.
    * auto.
    * auto.
- induction H.
  + constructor.
  + apply multi_R in H0. apply multi_trans with y.
    * auto.
    * auto.
Qed.
(** [] *)

(** **** Exercise: 5 stars, standard (determinstic_normalizing_is_strong_normalizing)

    Recall the definition of [normalizing] from above.
    A stronger property called "strong normalization" 
    requires that there aren't any infinite runs.
    To formally state is we use a version of [multi]
    that counts the number of steps: *)

Inductive multi_n {X : Type} (R : relation X) : nat -> relation X :=
  | multi_n_refl : forall (x : X), multi_n R 0 x x
  | multi_n_step : forall (x y z : X) n,
                    R x y ->
                    multi_n R n y z ->
                    multi_n R (S n) x z.

Definition strong_normalizing {X : Type} (R : relation X) :=
  forall t, exists n, forall t', ~ multi_n R n t t'.

(** Your task is to prove that if [R] is a deterministic
    relation, then normalization implies *strong* normalization. *)

Theorem determinstic_normalizing_is_strong_normalizing {X : Type} :
  forall (R : relation X), deterministic R ->
  normalizing R -> strong_normalizing R.
Proof.
Admitted.
(** [] *)

(** **** Exercise: 3 stars, standard (exists_normalizing_not_strong_normalizing)

    Second, give an example of a relation that is normalizing 
    but not strongly normalizing. You can freely choose the type
    [X] to make your solution as easy to verify as possible. *)
Inductive R_nat : nat -> nat -> Prop :=
  | R_step : forall x, R_nat (S x) x .

Notation "x 'o' y" := (R_nat x y) (at level 10).
Check normal_form.

(*      I TRIED TO PROOF THIS :(     *)
Lemma helper : forall n, multi_n R_nat n 0 0.
Proof. Admitted.
(*
Definition normal_form {X : Type}
              (R : relation X) (t : X) : Prop :=
  ~ exists t', R t t'.
*)
Theorem exists_normalizing_not_strong_normalizing :
  exists (X : Type) (R : relation X), 
  normalizing R /\ ~ strong_normalizing R.
Proof.
exists nat. exists R_nat. split.
  - unfold normalizing.
    intros. destruct t eqn:E.
    + exists 0. split. constructor. unfold normal_form. intros contra.
 destruct contra as [t' contra]. inversion contra.
    +  exists 0. split. subst. induction (S n). constructor. destruct n0.
apply multi_step with 0. constructor. constructor. apply multi_step with (S(n0)).
constructor. auto. unfold normal_form. intros contra.
 destruct contra as [t' contra]. inversion contra.

 - unfold strong_normalizing. intros H.
  specialize (H 0).
  destruct H as [n H].
  apply (H 0). apply helper.
Qed.
            
(** [] *)

(** **** Exercise: 5 stars, advanced, optional (strong_normalization)

    Try to prove the converse direction: 
    every strongly normalizing  is normalizing.
    We believe that (using our definitions of these terms) one needs 
    classical logic. So you are free to add excluded-middle
    as an axiom: Axiom EM : forall P, P \/ ~P.
 *)

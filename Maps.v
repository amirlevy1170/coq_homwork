...
- destruct (eqb_spec x2 x).  destruct H.
 + rewrite e,e0. reflexivity.
 + reflexivity.
- destruct (eqb_spec x2 x). + reflexivity. + reflexivity.
Qed.
...
(** **** Exercise: 3 stars, standard (remove) *)

(** A user interface we haven't defined above is the removal of keys
    from a map. Implement it in [remove] below. *)

Definition remove {A : Type} (x : string)
           (m : partial_map A) : partial_map A :=
( x !-> None;m).

(** Show that removing behaves as expected with the same key. *)

Lemma remove_eq : forall {A : Type} (m : partial_map A) x,
  remove x m x = None.
Proof.
intros. unfold remove. unfold t_update. destruct (eqb_spec x x).
- reflexivity.
- destruct n. reflexivity.
Qed.


(** Show that removing behaves as expected with another key. *)

Theorem remove_neq : forall {A : Type} (m : partial_map A) x1 x2,
  x2 <> x1 ->
  remove x2 m x1 = m x1.
Proof.
intros. unfold remove. unfold t_update. destruct (eqb_spec x2 x1).
- apply H in e. contradiction.
- reflexivity.
Qed. 

(** Show that the above two properties characterize removal. *)

Theorem remove_char : forall A x (f : partial_map A -> partial_map A),
  (forall m, f m x = None
      /\ forall x', x <> x' -> f m x' = m x'
  ) -> f = remove x.
Proof. 
intros A x f.
intros H.
...
apply functional_extensionality.
intros.
destruct (eqb_spec x x0) as [Hxy | Hxy].

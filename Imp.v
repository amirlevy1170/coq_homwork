...
Proof.
intros b.
induction b. try reflexivity. try reflexivity.
  repeat(simpl; rewrite optimize_0plus_sound;
    destruct a1;simpl;rewrite optimize_0plus_sound;reflexivity).
...
Inductive bevalR: bexp -> bool -> Prop :=
    ...
    | E_BEq  (e1 e2: aexp) (n1 n2: nat) : (e1 ==> n1) -> (e2 ==> n2) ->
                                          ((BEq e1 e2) ==>b (Nat.eqb n1 n2))
    | E_BNeq (e1 e2: aexp) (n1 n2: nat) : (e1 ==> n1) -> (e2 ==> n2) ->
                                          ((BNeq e1 e2) ==>b negb (Nat.eqb n1 n2))
    | E_BLe  (e1 e2: aexp) (n1 n2: nat) : (e1 ==> n1) -> (e2 ==> n2) ->
                                          ((BLe e1 e2) ==>b (Nat.leb n1 n2))
    | E_BGt  (e1 e2: aexp) (n1 n2: nat) : (e1 ==> n1) -> (e2 ==> n2) ->
                                          ((BGt e1 e2) ==>b negb (Nat.leb n1 n2))
    | E_BNot (e: bexp) (b: bool) : (e ==>b b) ->
      ((BNot e) ==>b (negb b))
    | E_BAnd (e1 e2: bexp) (b1 b2: bool) : (e1 ==>b b1) -> (e2 ==>b b2) ->
...
repeat(try discriminate). try discriminate. try discriminate. try discriminate.
try discriminate. try discriminate. try discriminate.
...
Fixpoint s_execute (st : state) (stack : list nat)
  ...
  | SLoad k :: t => s_execute st ((st k) :: stack)                                       t
  | SPlus     :: t => s_execute st (((hd 0 (tl stack)) + (hd 0 stack))::(tl (tl stack))) t
  | SMinus    :: t => s_execute st (((hd 0 (tl stack)) - (hd 0 stack))::(tl (tl stack))) t
  | SMult     :: t => s_execute st (((hd 0 (tl stack)) * (hd 0 stack))::(tl (tl stack))) t
  end.
...
Ltac help stack IH := simpl;destruct stack as [| n [| m stack']];apply IH.
Ltac help2 IH := simpl;apply IH.
Theorem execute_app : forall st p1 p2 stack,
  s_execute st stack (p1 ++ p2) = s_execute st (s_execute st stack p1) p2.
Proof.
  intros st p1 p2 stack.
  generalize dependent stack.
  induction p1 as [| h t IH].
...
Ltac helper IHe1 IHe2 :=     try reflexivity;
...
Module BreakImp.
(** **** Exercise: 4 stars, advanced, optional (break_imp)
...
(** **** Exercise: 5 stars, advanced (multi_assignment)

    Some languages allow "multi assignment" that assigns new values
    to more than one variable in a single command.
    Below, you can find two different semantics for such command.
    Your exercise is to show a command that evaluates differently
    in the two semantics, and prove your claim. 

    For simplicity, we omit conditionals and loops from the language. *)
...
Lemma contradiction2 : (X !-> 1; Y !-> 0) <> (X !-> 1; Y !-> 1).
Proof.
  unfold not. intros contra.
  assert (H_Y_eq : (X !-> 1; Y !-> 0) Y = (X !-> 1; Y !-> 1) Y).
  { rewrite contra. reflexivity. }
  simpl in H_Y_eq. discriminate H_Y_eq.
Qed.

Lemma states_neq : st1 <> st2.
Proof.
unfold not, st1, st2. intros. apply contradiction2 in H. assumption.
Qed.

Lemma ceval_example1:
  st =[ c ]=> st1.
Proof.
(* FILL IN HERE *) Admitted.
...
Lemma multi_assignment :
  st1 <> st2 /\ st =[ c ]=> st1 /\ st =[[ c ]]=> st2.
Proof.
(* FILL IN HERE *) Admitted.
